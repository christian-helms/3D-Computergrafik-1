...
export class TriangleFanGeometry extends Geometry {
    ...
    public build(numOuterPoints: number, radius: number): void {
        ...
        for (let i = 0; i < numOuterPoints; i++) {
            vertices.push(radius * Math.cos(i * 2 * Math.PI / numOuterPoints) / Math.cos(Math.PI / numOuterPoints));
            vertices.push(radius * Math.sin(i * 2 * Math.PI / numOuterPoints) / Math.cos(Math.PI / numOuterPoints));
        }
