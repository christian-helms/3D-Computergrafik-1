...
export class ModelPass extends Initializable {
    ...
    protected _calculated: mat4;
    protected _comparison: mat4;
    protected _interpolateFactor: number;
    ...
    public initialize(): boolean {
        ...
        vert.initialize(require('./rotation.vert'));
        const frag = new Shader(this._context, this._gl.FRAGMENT_SHADER);
        frag.initialize(require('./rotation.frag'));
    ...
    public frame(): void {
        ...
        const xPos = this._interpolateFactor * 2 - 1;
        const yPos = 0.75;
        this.setMat([xPos, yPos, 0], this._scale, this._calculated);
        this._scenePass.frame();

        // this._program.bind();
        if(this._comparison) {
            this.setMat([xPos, -yPos, 0], this._scale, this._comparison);
            this._scenePass.frame();
        }
    ...
    public set calculated(m: mat4) {
        this._calculated = m;
    }

    public set comparison(m: mat4) {
        this._comparison = m;
    }

    public set interpolateFactor(i: number) {
        this._interpolateFactor = i;
    }
